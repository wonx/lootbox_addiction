...
scheduler.add_job(run_generate_df_purchases, 'interval', minutes=10, id='generate_df_purchases', replace_existing=True, max_instances=1)
scheduler.add_job(run_generate_df_purchases_value, 'interval', minutes=30, id='generate_df_purchases_value', replace_existing=True, max_instances=1)
scheduler.add_job(run_variable_harmonization, 'cron', day_of_week='*', hour=0, minute=2, id='variable_harmonization', replace_existing=True, max_instances=1)
scheduler.add_job(run_ML_addiction_predictions, 'cron', day_of_week='mon', hour=0, minute=15, id='ML_addiction_predictions', replace_existing=True, max_instances=1)
